...
// Fixed local EventCard component
function EventCard({ event }) {
  return (
    <Link href={`/events/${event.id}`}>
      <Card className="overflow-hidden shadow-sm hover:shadow-md transition-all cursor-pointer group h-full">
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchCategoryStats = async () => {
      try {
        const response = await fetch(`${baseUrl}/api/stats/categories`);
    ...
    // 3. Fetch Trending/Top Events (from your events controller)
    const fetchTrendingEvents = async () => {
      try {
        const response = await fetch(`${baseUrl}/api/events/trending`);
    ...
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        <Card className="shadow-sm border-muted">
          ...
          <CardContent>
            {isLoadingStats ? <Skeleton className="h-8 w-24" /> : (
              <div className="text-3xl font-black">${stats?.totalRevenue || "0.00"}</div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="shadow-sm border-muted">
          ...
          <CardContent>
            {isLoadingStats ? <Skeleton className="h-8 w-16" /> : (
              <div className="text-3xl font-black">{stats?.totalOrders || 0}</div>
            )}
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {isLoadingTrending ? (
            ...
            trendingEvents.slice(0, 4).map((event) => (
             ...
             <div className="col-span-full py-8 text-center border-2 border-dashed rounded-2xl bg-muted/20 text-muted-foreground">
